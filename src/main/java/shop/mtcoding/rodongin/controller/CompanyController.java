...
import shop.mtcoding.rodongin.dto.company.CompanyRes.CompanyDetailResDto;
...
public class CompanyController {
    ...
    @RequestBody CompanyDetailResDto companyDetailResDto){
        Company comPrincipal = (Company) session.getAttribute("comPrincipal");
        if (comPrincipal == null) {
            throw new CustomApiException("인증이 되지 않았습니다", HttpStatus.UNAUTHORIZED);
        }
        if (companyDetailResDto.getCompanyEstablish() == null){
            throw new CustomApiException("Etablish를 작성해주세요");
        } 
        if (companyDetailResDto.getCompanyFullname() == null || companyDetailResDto.getCompanyFullname().isEmpty()) {
            throw new CustomApiException("Fullname을 작성해주세요");
        }       
        if (companyDetailResDto.getCompanyIntroduction() == null || companyDetailResDto.getCompanyIntroduction().isEmpty()) {
            throw new CustomApiException("Introduction을 작성해주세요");
        }
        if (companyDetailResDto.getCompanyHistory() == null || companyDetailResDto.getCompanyHistory().isEmpty()) {
            throw new CustomApiException("History를 작성해주세요");
        }
        if (companyDetailResDto.getCompanyVision() == null || companyDetailResDto.getCompanyVision().isEmpty()) {
            throw new CustomApiException("Vision을 작성해주세요");
        }

        companyService.기업소개등록(companyDetailResDto, comPrincipal.getId());
